...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRect(bb.Min, bb.Max, colBorder, 0.f, 15, 1.f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (label_size.x > 0.0f) {
        ...
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + center), label);
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    ImU32 frame_col1 = GetColorU32(ImGuiCol_FrameBg);
    ImU32 frame_col2 = GetColorU32(ImGuiCol_FrameBgActive);
    const float value_x2 = ImMax(frame_bb.Min.x, frame_bb.Max.x - arrow_size);
    RenderNavHighlight(frame_bb, id);
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilledMultiColor(frame_bb.Min, ImVec2(value_x2, frame_bb.Max.y), frame_col1, frame_col1, frame_col2, frame_col2);
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ImU32 bg_col = GetColorU32(ImGuiCol_FrameBg);
        ImU32 bg_col2 = GetColorU32(ImGuiCol_FrameBgActive);

        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        window->DrawList->AddRectFilledMultiColor(ImVec2(value_x2, frame_bb.Min.y), frame_bb.Max, bg_col, bg_col, bg_col2, bg_col2);

     //   window->DrawList->AddRectFilledMultiColor(ImVec2(value_x2, frame_bb.Min.y), frame_bb.Max, bg_col, bg_col, bg_col2, bg_col2, style.FrameRounding, (w <= arrow_size) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Right);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const auto colBorder = ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_Border));
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, colBorder, 0.f, 15, 1.f);
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + center), label);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
   ...
   //draw_list->AddCircleFilled(sv_cursor_pos, 2, ImColor(1,1,1,255), 255);
    draw_list->AddRectFilled(ImVec2(sv_cursor_pos.x - 1, sv_cursor_pos.y - 1), ImVec2(sv_cursor_pos.x + 1, sv_cursor_pos.y + 1), ImColor(255, 255, 255, 255));
    draw_list->AddRect(ImVec2(sv_cursor_pos.x - 2, sv_cursor_pos.y - 2), ImVec2(sv_cursor_pos.x + 2, sv_cursor_pos.y + 2), ImColor(1, 1, 1, 255));
...
//    draw_list->AddCircle(sv_cursor_pos, sv_cursor_rad, col_white, 12);
    ...
    if (alpha_bar)
    {
        ...
        RenderArrowsForVerticalBar(draw_list, ImVec2(bar1_pos_x - 1, bar1_line_y), ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz), bars_width + 2.0f, 255);
    }
